...
let ChatBotService = ChatBotService_1 = class ChatBotService {
  ...
  Nguyên tắc:
    ...
    async loadProductData() {
        ...
        this.logger.log(`Đã tạo embedding cho ${this.productData.length} sản phẩm.`);
    }
    formatProductInfo(product, category, brand) {
        ...
        const firstImage = product.variants?.find((variant) => variant.color?.length > 0)?.color[0]?.images[0];
...
`}
    ...
    getNameFromPopulatedField(field) {
...
Hãy trả lời dựa vào thông tin sản phẩm trên. Nếu có nhiều sản phẩm phù hợp, hãy so sánh và đưa ra lời khuyên. Luôn hiển thị hình ảnh của sản phẩm được đề cập.
    ...
    async getEmbedding(text) {
        try {
            ...
            if (!Array.isArray(vector))
                throw new Error('Không tạo được vector.');
            return vector;
        }
        catch (error) {
    ...
    findRelevantProducts(inputVector, topK = 5) {
    ...
    calculateCosineSimilarity(a, b) {
        if (a.length !== b.length)
            throw new Error('Vector không cùng chiều.');
    ...
    findProductsByCategory(categoryName) {
        return this.productData.filter((product) => product.category.toLowerCase().includes(categoryName.toLowerCase()));
    }
    findProductsByBrand(brandName) {
        return this.productData.filter((product) => product.brand.toLowerCase().includes(brandName.toLowerCase()));
    }
...
__decorate([
    (0, mongoose_1.InjectModel)(product_schema_1.Products.name),
    __metadata("design:type", Object)
], ChatBotService.prototype, "ProductModel", void 0);
